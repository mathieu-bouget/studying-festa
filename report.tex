...
\begin{center}
	...
	\begin{tikzcd}[sep=5em]
E'_0 \arrow[d, "\varphi'"] & E_0 \arrow[d, "\varphi"', shift right] \arrow[l, "\omega", bend left] \arrow[l, "\sigma_0"', bend right] \\
...
\subsection{Attack description for FESTA}
...
\subsubsection{Input}
	...
	\begin{itemize}
		\item The endomorphism $\widehat\sigma_0\circ\omega\in\mathrm{End}(E_0)$ should admit $P,Q$ as eigenvectors
...
An isogeny $\varphi:E_0\to E$ of degree $d$ and a scaling matrix $\mathbf A$ such that
\[
\begin{pmatrix} S \\ T \end{pmatrix} = \mathbf A\cdot\varphi\begin{pmatrix} P \\ Q \end{pmatrix} 
\] 
...
\begin{algorithm}
	...
	Compute maximum $u|d$ such that $\psi([u]I)=0$ \;
	Compute maximum $v|d$ such that $\psi([v]J)=0$ \;
	...
	%TODO vérifier mathématiquement ce truc

	Compute $P\in\langle I,J\rangle$ of order $d$ such that $\psi(P)=0$\;
	Construct the isogeny $\varphi_{d/d'}$ of kernel  $\langle [d']P\rangle$\;
	...
	$res\gets\bot$\;
	$b\gets 0$\;
	$P',Q'\gets \varphi_{d/d'}(P),\varphi_{d/d'}(Q)$\;
	\While{$res\neq\top$ and $b<2^{r'}$}{
	...
	\If{$res=\top$}{
		\Return  $\varphi$ and matrix $\mathrm{Diag}(\alpha_b,\beta_b)$ where $\varphi$ is the isogeny with $\varphi_b$ as  $d'$ component and $\varphi_{d/d'}$ as $d/d'$
		component \;
	}\Else{
		\Return $\bot$
	}
...
%First reduce the generators to get generators of $E[d]\cup\ker(\psi)$, look at the divisors that appear twice in generator orders list.



\subparagraph{degree-$d/d'$ component}
Pick an arbitrary element $U$ of $\mathrm{ker}(\psi)$ of order $d$ and scale it by $d'$ to get $d'U$,
then the group generated by $d'U$ is a subgroup of $\mathrm{ker}(\widehat\varphi)$.

\subparagraph{degree-$d'$ component}
We now want to find the degree-$d'$ component of $\varphi$, that is a cyclic subgroup of order $d'$ in the group $E_0[d']\cong \mathbb{Z}_{d'}\times \mathbb{Z}_{d'}$.
...
For each $i$, find two generators $L_{i,0}$ and $L_{i,1}$ of the two eigenspaces of $\widehat\sigma_0\circ\omega$ acting on $E_0[l_i^{e_i'}]$. %With Weil pairing we have the double discret log so we can recover the matrix of $\widehat\sigma_0\circ\omega$ in  $E_0[l_i^{e'_i}]$
...
\begin{itemize}
	...
	\item Compute $p_b=\varphi_b(P)$ and $q_b=\varphi_b(Q)$ with $\varphi_b$ the isogeny such that, for each $1\leq i\leq r'$, $\ker(\varphi_b)[l_i^{e'_i}]=\langle L_{i,b[i]}\rangle$
...
%	\item Compute a guess $\varphi_g$ for the secret isogeny
	...
	\item Check if $\det A=u_b v_v$, then the guess is correct, otherwise take next $b$ and make a new guess
\end{itemize}
	 It is possible to simplify the computation of the guesses $\varphi_b$ using product trees in the isogeny chain
...
%	\item if $s$ and $N$ are coprime, compute $\psi\begin{pmatrix} S \\ T \end{pmatrix}$ % whatif only partial information
