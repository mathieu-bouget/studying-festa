...
(Add conditions of success with N,d…)
...
For each $i$, find two generators $L_{i,0}$ and $L_{i,1}$ of the two eigenspaces $\widehat\sigma_0\circ\omega$ acting on $E_0[l_i^{e_i'}]$
...
\begin{itemize}
	...
	\item Compute  $u=\log_{\varphi_g(P)}(S)$ and $v=\log_{\varphi_g(Q)}(T)$
	\item Check if $\det A=uv$, then the guess is correct, otherwise take next $b$ and make a new guess
\end{itemize}
