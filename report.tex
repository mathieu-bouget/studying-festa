...
\paragraph{Output}
Secret isogeny $\varphi:E_0\to E$ and diagonnal scaling matrix $\mathbf A$ which is here assumed to be diagonnal.
\subsubsection{Input}
	\paragraph{Description of the CIST instance}
	We are given the values that describe the CIST instance to solve : the starting curve $E_0$ and the codomain $E$, the degree $d$ of the secret isogeny $\varphi$. We also have $N$, a basis $\{P,Q\}$ of $E_0[N]$ the $N$-torsion of the starting curve and also scaled information of how $\varphi$ acts on it, that is points $S,T$ such that \[
	\begin{pmatrix} S \\ T \end{pmatrix} =\mathbf A \cdot\varphi\begin{pmatrix} P \\ Q \end{pmatrix} 
	\] where $\mathbf A$ is the secret scaling matrix.
	All these inputs are $E_0,E,d,N,P,Q,S,T$.

	\paragraph{Additionnal inputs}
	The attack also require to find good $\sigma_0$ and good $\omega$, these are isogeny going from $E_0$ and having the same codomain $E_0'$ and that must fullfil followings requirements
	\begin{itemize}
		\item The endomorphism $\widehat\sigma_0\circ\omega\in\mathrm{End}(E_0)$ should admitt $P,Q$ as eigenvectors
		\item The degree $w$ of $\omega$ should be small
		\item The pushforward of $\sigma_0$ through $\varphi$ should be easy to compute
...
\paragraph{Compute $\sigma$} We compute $\sigma$ the pushforward through $\varphi$.
