...
\begin{algorithm}
	\DontPrintSemicolon
	\KwIn{$E_0,E,d,N,P,Q,S,T,\sigma_0,s,\omega,w$}
...
\paragraph{Compute $\sigma$} We compute $\sigma$ the pushforward of $\sigma_0$ through $\varphi$.
...
%Thanks to Lemma 3 we have an SIDH instance on $\psi=\varphi'\circ\sigma_0\circ\widehat\varphi$ with  $\varphi'$ the pushforward of $\varphi$ through $\sigma$

%(Add conditions of success with N,d…)%TODO

\paragraph{Compute $\varphi$}
...
\begin{algorithm}
	...
	\If{$res=\top$}{
		\Return  $\varphi$ where $\varphi$ is the isogeny with $\varphi_b$ as  $d'$ component and $\varphi_{d/d'}$ as $d/d'$
		component \;
	}\Else{
		\Return $\bot$
	}
